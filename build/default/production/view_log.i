...
void view_logs(void)
{
    ...
    while(1)
    {
        log_key = read_digital_keypad(0);
        if(no_event)
        {
            clear_clcd();
            clcd_print(" NO EVENTS ",(0x80 + 0));
        ...
        else
        {
            ...
            else if(log_key == 0x3F && !long_press1)
            {
                long_press1 = 1;
                if(press4 < 10)
                {
            ...
            else if(log_key == 0x3F && !long_press2)
            {
                long_press2 = 1;
                if(press5 < 10)
                {
...
static void view_log_events(unsigned char log_no)
{
    ...
    clcd_putch('#',(0x80 + 0));
    clcd_putch(log_no+48,(0xC0 + 0));
    clcd_print("  TIME    E  SP",(0x80 + 1));

    clcd_print(view_log,(0xC0 + 2));
}
